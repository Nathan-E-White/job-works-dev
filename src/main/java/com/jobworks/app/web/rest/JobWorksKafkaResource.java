...
@RestController
@RequestMapping("/api/job-works-kafka")
public class JobWorksKafkaResource {

    private final Logger log = LoggerFactory.getLogger(JobWorksKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public JobWorksKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
