...
import com.jobworks.app.config.EmbeddedKafka;
import com.jobworks.app.config.KafkaSseConsumer;
import com.jobworks.app.config.KafkaSseProducer;
...
class JobWorksKafkaResourceIT {
    ...
    void producesMessages() throws InterruptedException {
        client.post().uri("/api/job-works-kafka/publish?message=value-produce").exchange().expectStatus().isNoContent();
    ...
    void consumesMessages() {
        ...
        String value = client
            .get()
            .uri("/api/job-works-kafka/consume")
